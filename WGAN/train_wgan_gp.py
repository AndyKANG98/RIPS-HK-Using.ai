...
# Here write the name of the image folder and 
# the size of those images (assume that they are square)
input_img_size = 150
img_folder = 'new_img'
...
img_paths = glob.glob('../data/wheel/%s/*.jpg' %img_folder)
data_pool = utils.DiskImageData(img_paths, batch_size, shape=[input_img_size, input_img_size, 3], preprocess_fn=preprocess_fn)
...
# summary writer
summary_writer = tf.summary.FileWriter('./summaries/wgan_gp_' + img_folder, sess.graph)

''' initialization '''
ckpt_dir = './checkpoints/wgan_gp_' + img_folder
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        if (it + 1) % 100 == 0:
            ...
            save_dir = './sample_images_while_training/wgan_gp_'+ img_folder
