...
from utils import eval_util as eval_util
...
with detection_graph.as_default():
	with tf.Session(graph=detection_graph) as sess:
		...
		for image_path, result_path in zip(TEST_IMAGE_PATHS, RESULT_IMAGE_PATHS):
			...
			# Transform the boxes' location and score to a dictionary
			for idx in range(np.squeeze(boxes).shape[0]):
				if (np.squeeze(boxes)[idx,:]).any():
					pred_boxes['box'+str(idx)] = {}
					pred_boxes['box'+str(idx)]['boxes'] = np.squeeze(boxes)[idx,:]
					pred_boxes['box'+str(idx)]['scores'] = np.squeeze(scores)[idx]

			# Obtain the ground truth boxes
			gt_boxes = eval_util.label_image(name)
			
			for i in np.arange(0.5,1,0.05):	
				img_results[i][name] = eval_util.get_single_image_results(gt_boxes, pred_boxes, i)
			...
			print('Images Processed: %d \r' % count)

		for i in np.arange(0.5,1,0.05):
			prec += eval_util.calc_precision_recall(img_results[i])[0]
	 		recall  += eval_util.calc_precision_recall(img_results[i])[1]
		...
		r = recall / 10
		t = (end - start) / count
		print('Precision: %f, Recall: %f, Execution time (per image): %fs' % (p,r,t))
